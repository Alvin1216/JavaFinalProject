...
public class set_image_by_board_16 {
	...
	public set_image_by_board_16(GraphicsContext gc,int[][] gameboard,Image[] image){
		this.gc=gc;
	...
	public void draw_image_background(where_position[][] wp){
		...
		for(int i=0;i<2;i++){
			for(int j=0;j<8;j++){
				gc.drawImage( image[0], start_x+(width+gap)*(j), start_y+(height+gap)*(i) );
				wp[i][j].set_position((int)(start_x+(width+gap)*(j)), (int)(start_y+(height+gap)*(i)) );
			}
		}
	}
